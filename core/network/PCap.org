* Wrapping libpcap
~libpcap~ is by far the /worst/ C libray I have ever wrapped. C lacks exceptions, so there isn't a /standard/ way to
signal errors. Instead, each C libray devises its own error handling scheme. This can be returning ~0~ to signal success
and not ~0~ for failure. This can mean returning an address on success and ~nullptr~ (C ~NULL~) on failure. It can use
an out-of-band ~errno~ value. Or it can require an explicit error-check function. If there is an error, /which/ error
can be documented by giving the user a ~char*~ pointing at a description of it. Or it can be a simple numeric value
(like ~errno~ provides), or the user can provide a buffer and then call a function to fill it with the error
description. Any of these is fine. Once you figure out what scheme is used, you create a wrapper than ensures the call
worked or signals an error however your application expects (~exceptions~, ~std::expected~, /et cetera/). The problem?

~libpcap~ uses /all/ of those mechanisms, seemingly at random. There's simply no /good/ way to make a general purpose
C-style wrapper around it. So Here's an RAII class that's still ackward, but at least contains the mess.

** TODO Use C++26 Reflection for errbuf
The reflection support, when it lands, will allow verifying the last argument /name/ is ~errbuf~, but for now we just
check if it's a mutable ~char*~.

#+begin_src c++ :tangle PCap.cppm
...
#+end_src
** Implicit pcap_t*
For some functions, the first parameter is our pcap pointer. In those cases, we supply it automatically. This means
detecting the /first/ parameter is a pcap_t*
...
#+end_src
** PCap
This is the actual resource handler. It gets through ~pcap_init~ and ~pcap_create~ on construction, then allows C-like
usage via ~PCapCall~, with the ~pcap_t*~ and ~errbuf~ supplied to the functions which require them. Error handling is
unified into a single exception mechanism.
